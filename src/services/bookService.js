...
exports.createBook = async (authorId, bookData, files) => {
    ...
    try {
        // Handle file uploads
        if (files?.coverImage) coverImage = await uploadCoverImage(files.coverImage[0].buffer);
        ...
        if (coverImage?.publicId) await deleteFile(coverImage.publicId, 'image').catch(console.error);
        if (pdfFile?.publicId) await deleteFile(pdfFile.publicId, 'raw').catch(console.error);
...
exports.getAllBooks = async (query) => {
    ...
    if (query.search) { //Full-text search
...
exports.getBooksByAuthorId = async (authorId, query) => {
    ...
    const books = await features.query
        .populate('author', 'firstName lastName profile.avatar authorInfo.biography')
        .populate('category', 'name');
...
exports.updateBook = async (bookId, userId, updateData, files, isAdmin = false) => {
    const book = await Book.findById(bookId);
    ...
    if (!isAdmin && !book.canEdit()) {
        throw new AppError(400, 'Cannot edit book in current status. Only draft or rejected books can be edited.');
    }

    // Handle file updates
    if (files?.coverImage) {
        // Delete old cover if exists
        if (book.coverImage?.publicId) await deleteFile(book.coverImage.publicId, 'image');
    ...
    if (files?.pdfFile) {
        ...
        // Regenerate preview
        if (updateData.pdfFile.publicId) updateData.preview = generatePreviewUrl(updateData.pdfFile.publicId, 10);
    }
...
exports.deleteBook = async (bookId, userId, isAdmin = false) => {
    ...
    // Check ownership (unless admin)
    if (!isAdmin && book.author.toString() !== userId.toString()) throw new AppError(403, 'You can only delete your own books');

    // Authors can only delete draft books
    if (!isAdmin && book.status !== 'draft') throw new AppError(400, 'You can only delete draft books. Contact admin for other cases.');
...
exports.submitForReview = async (bookId, userId) => {
    ...
    if (book.author.toString() !== userId.toString()) throw new AppError(403, 'You can only submit your own books');

    if (book.status !== 'draft' && book.status !== 'rejected') throw new AppError(400, 'Only draft or rejected books can be submitted for review');

    // Validate required fields for submission
    if (!book.pdfFile?.url) throw new AppError(400, 'PDF file is required before submitting for review');
...
exports.approveBook = async (bookId, adminId) => {
    ...
    if (book.status !== 'pending') throw new AppError(400, 'Only pending books can be approved');
...
exports.rejectBook = async (bookId, adminId, rejectionReason) => {
    ...
    if (book.status !== 'pending') throw new AppError(400, 'Only pending books can be rejected');
...
exports.getPreviewUrl = async (bookId) => {
    ...
    if (!book.pdfFile?.publicId) throw new AppError(404, 'No preview available for this book');
